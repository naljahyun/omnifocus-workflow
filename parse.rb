...
now = Date.today
matcher = /^((이달|이번달|담달|다음달|(내년|[0-9]{4}년){0,1} *[0-9]+월){0,1} *[0-9]+일+|오늘|내일|모레|(이번주|담주|다음주|다담주|다다음주){0,1} *([월화수목금토일](요일|욜)))( *(새벽|아침|점심|오전|오후|저녁|밤){0,1} *([0-9]+시) *([0-9]+분|반){0,1}){0,1}에{0,1} */u
...
if schedule_string =~ matcher
	...
	if month and day
		sentence = "#{subject} %02d/%02d/%02d" % [year, month, day]
		sentence += " %02d:%02d" % [hour, minute] if hour and minute
	end
end
